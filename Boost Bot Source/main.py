...
ctypes.windll.kernel32.SetConsoleTitleW("NT Services Boost bot")

activity = discord.Activity(type=discord.ActivityType.watching, name="NT Services")
...
@bot.event
async def on_ready():
    activity = discord.Game(name=".gg/retroboosts", type=2)
    await bot.change_presence(activity=discord.Activity(type=discord.ActivityType.competing, name=".gg/retroboosts"))
    print(f"                 Retro Boosts")
    print(f"              .gg/retroboosts")
    print(f"\n       [ + ] Discord bot status | [{Fore.GREEN}Online{Fore.RESET}]")
    print(f"       [ + ] Sellix intergration | [{Fore.RED}Soon{Fore.RESET}]")
    print(f"           [ + ] Proxy status | [{Fore.GREEN}Online{Fore.RESET}]")
...
async def stock(ctx: discord.ApplicationContext):
    if not isWhitelisted(ctx):
        ...
        embed=discord.Embed(title="__Stock__", description=f"\n\
        **Tokens: {len(open('tokens.txt', encoding='utf-8').read().splitlines())} **\n\
        **Boosts: {len(open('tokens.txt', encoding='utf-8').read().splitlines())* 2}**", color=discord.Color.from_rgb(106, 13, 173))
        embed.set_footer(text = f"discord.gg/retroboosts", icon_url = "")
...
async def restock(ctx: discord.ApplicationContext, code: discord.Option(str, "toptal.com paste code.", required=True)):
    ...
    paste = requests.get(f"https://www.toptal.com/developers/hastebin/documents/{code}", headers={
        ...
        "Referer": f"https://www.toptal.com/developers/hastebin/{code}",
    ...
    return await ctx.edit(content=f"```{paste}```\n\n*Added to stock.*")
...
async def boost(ctx: discord.ApplicationContext,
    ...
    await ctx.respond("Started boosting .gg/{invitecode}")
    ...
    return await ctx.edit(content=f"*Finished boosting .gg/{INVITE}!*")
